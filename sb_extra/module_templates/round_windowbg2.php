...
function sb_template_round_windowbg2($module, $style, $location = 0)
{
	...
	if (empty($style))
	{
		...
		if (!empty($module['header_display']) || $module['header_display'] == 2)
			echo '
				<div id="sb_module_', $module['type'], '_', $module['id'], '" class="', (!$module['is_collapsed'] || empty($modSettings['sb_collapse_modules']) ? ' block_header' : ''), '"', (!empty($location) ? ' style="margin-top: 7px;"' : ''), '>
					<h3 class="sb_plain">
						', !empty($modSettings['sb_collapse_modules']) && $module['header_display'] != 2 ? '<img class="sb_curveblock floatright hand" id="' . $module['type'] . 'collapse_' . $module['id'] . '" src="' . $settings['images_url'] . '/collapse.gif" alt="" title="' . $txt['sb_core_modules'] . '" />' : '', '
		...
		if (!empty($module['header_display']))
			echo '
							<div', (!empty($location) ? ' style="margin-top: 7px;"' : ''), '>
								<h3 class="sb_plain">
					', !empty($modSettings['sb_collapse_modules']) && $module['header_display'] != 2 ? '<img class="floatright" id="' . $module['type'] . 'collapse_' . $module['id'] . '" src="' . $settings['images_url'] . '/collapse.gif" alt="" title="' . $txt['sb_core_modules'] . '" />' : '', '
					' . $module['module_icon'] . $module['module_title'] . '
								</h3>
							</div>';

		echo '
							<div id="', $module['type'], 'module_', $module['id'], '" style="', $module['is_collapsed'] && !empty($modSettings['sb_collapse_modules']) && !empty($module['header_display']) ? ' display: none;' : '', '">
								', !empty($module['fields']) ? $module['function']($module['fields']) : $module['function'](), '
							</div>';
	}
}
